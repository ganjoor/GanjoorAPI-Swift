...
extension Controller {
    public func categories(request: RouterRequest, response: RouterResponse, next: @escaping () -> Void) throws {
        ...
        response.status(.OK).send(json: JSON(["categories": connection.mappedResults, "count": connection.mappedResults.count]))
        next()
    }
    ...
    public func category(request: RouterRequest, response: RouterResponse, next: @escaping () -> Void) throws {
        do {
            ...
            response.status(.OK).send(json: JSON(["category": connection.mappedResults, "count": connection.mappedResults.count]))
        }catch let err {
...
extension Controller {
    public func poets(request: RouterRequest, response: RouterResponse, next: @escaping () -> Void) throws {
        ...
        response.status(.OK).send(json: JSON(["poets": connection.mappedResults, "count": connection.mappedResults.count]))
        next()
    }
    ...
    public func poet(request: RouterRequest, response: RouterResponse, next: @escaping () -> Void) throws {
        do {
            ...
            response.status(.OK).send(json: JSON(["poet": connection.mappedResults, "count": connection.mappedResults.count]))
        }catch let err {
            response.status(.noContent).send("\(err)")
        }
        next()
...
extension Controller {
    public func poemsByCategory(request: RouterRequest, response: RouterResponse, next: @escaping () -> Void) throws {
        do {
            ...
            response.status(.OK).send(json: JSON(["poems": connection.mappedResults, "count": connection.mappedResults.count]))
        }catch let err {
            response.status(.noContent).send("\(err)")
        }
        next()
...
extension Controller {
    public func verses(request: RouterRequest, response: RouterResponse, next: @escaping () -> Void) throws {
        do {
            ...
            response.status(.OK).send(json: JSON(["verses": connection.mappedResults, "count": connection.mappedResults.count]))
        }catch let err {
            response.status(.noContent).send("\(err)")
        }
        next()
